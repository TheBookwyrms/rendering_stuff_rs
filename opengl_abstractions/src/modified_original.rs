...
fn create_shader(opengl:&Gl, shader_type:ShaderType) -> u32 {
    let shader_id =  unsafe {
        match shader_type {
            ShaderType::VertexShader   => Ok(create_shader(opengl, gl::VERTEX_SHADER)),
            ShaderType::FragmentShader => Ok(create_shader(opengl, gl::FRAGMENT_SHADER)),
            _ => Err(0)
        }
    };
...
fn buffer_bind(opengl:&Gl, target:gl::types::GLenum, buffer:u32) {
    unsafe { opengl.BindBuffer(target, buffer) }
}

fn bind_buffer(opengl:&Gl, target:GlSettings, buffer:u32) {
    match target {
        GlSettings::ArrayBuffer => buffer_bind(opengl, gl::ARRAY_BUFFER, buffer),
...
fn buffer_data(opengl:&Gl, target:gl::types::GLenum, size:gl::types::GLsizeiptr,
    data_ptr:*const gl::types::GLvoid, draw_type:gl::types::GLenum) {
        ...
        GlSettings::ArrayBuffer => {
            match draw_type {
                GlSettings::StaticDraw => unsafe {
                    buffer_data(opengl, gl::ARRAY_BUFFER, size, data_ptr, gl::STATIC_DRAW)
                },
                GlSettings::StreamDraw => unsafe {
                    buffer_data(opengl, gl::ARRAY_BUFFER, size, data_ptr, gl::STREAM_DRAW)
                },
                GlSettings::DynamicDraw => unsafe {
                    buffer_data(opengl, gl::ARRAY_BUFFER, size, data_ptr, gl::DYNAMIC_DRAW)
                },
...
fn buffer_sub_data(opengl:&Gl, target:gl::types::GLenum, size:isize, data:*const c_void) {
...
fn buffer_sub_data(opengl:&Gl, target:GlSettings, size:isize, data:*const c_void) {
    match target {
        GlSettings::ArrayBuffer => 
        buffer_sub_data(opengl, gl::ARRAY_BUFFER, size, data),
        _ => Err("invalid buffer target").unwrap(),
    }
}    


fn point_size(opengl:&Gl, size:f32) {
    unsafe { opengl.PointSize(size) }
}

fn draw_arrays(opengl:&Gl, mode:gl::types::GLenum, num_shapes:i32) {
    unsafe { opengl.DrawArrays(mode, 0, num_shapes) }
}


fn draw_arrays(opengl:&Gl, mode:GlSettings, num_shapes:i32) {
    point_size(opengl, 10.0);
    match mode {
        GlSettings::GlPoints =>    unsafe {draw_arrays(opengl, gl::POINTS, num_shapes)},
        GlSettings::GlLines =>     unsafe {draw_arrays(opengl, gl::LINES, num_shapes)},
        GlSettings::GlTriangles => unsafe {draw_arrays(opengl, gl::TRIANGLES, num_shapes)},
